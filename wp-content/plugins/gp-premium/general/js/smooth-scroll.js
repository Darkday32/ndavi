...
var scroll = new SmoothScroll( smooth.elements.join(), {
	...
	offset: function( anchor, toggle ) {
		var nav = document.querySelector( '.main-navigation:not(.slideout-navigation)' ),
			stickyNav = document.querySelector( '.navigation-stick' ),
			openMobileMenu = document.querySelector( '.main-navigation.toggled' ),
			offset = 0;

		if ( stickyNav && ! stickyNav.classList.contains( 'auto-hide-sticky' ) ) {
			offset = offset + stickyNav.clientHeight;
		}

		if ( openMobileMenu && openMobileMenu.contains( toggle ) ) {
			offset = offset + nav.clientHeight - nav.querySelector( '.menu-toggle' ).clientHeight;
		}

		return offset;
	},
	before: function() {
		// Removes focus from dropdown menus.
