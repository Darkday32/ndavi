...
function perfmatters_rest_authentication_errors($result) {
	if(!empty($result)) {
		...
		if($perfmatters_options['disable_rest_api'] == 'disable_completely') {
			$disabled = true;
		}
		elseif($perfmatters_options['disable_rest_api'] == 'disable_non_admins' && !current_user_can('manage_options')) {
			$disabled = true;
		}
...
function perfmatters_script_manager_print_section($category, $group, $scripts) {
	...
	if(isset($options['disabled'][$category][$group]['everywhere']) || (isset($options['disabled'][$category][$group]['current']) && in_array($currentID, $options['disabled'][$category][$group]['current'])) || !empty($options['disabled'][$category][$group]['regex'])) {
		$statusDisabled = true;
	}
...
function perfmatters_script_manager_print_status($type, $handle) {
	...
	$statusDisabled = false;
	if(isset($options['disabled'][$type][$handle]['everywhere']) || (isset($options['disabled'][$type][$handle]['current']) && in_array($currentID, $options['disabled'][$type][$handle]['current'])) || !empty($options['disabled'][$type][$handle]['regex'])) {
		$statusDisabled = true;
	}
