...
function perfmatters_settings() {
    ...
    /*add_settings_field(
        ...
        perfmatters_title(__('Additional Parameters', 'perfmatters'), 'query_string_parameters') . perfmatters_tooltip(''), 
        'perfmatters_print_input', 
        'perfmatters_options', 
        'perfmatters_options', 
        array(
    ...
    add_settings_field(
    	...
    	array(
    		...
    		'options' => array(
    			...
    			'disable_completely' => __('Disable Completely', 'perfmatters'),
    ...
    add_settings_field(
        ...
        array(
            'id' => 'preconnect',
            'option' => 'perfmatters_extras',
            'tooltip' => __('Preconnect allows the browser to set up early connections before an HTTP request, eliminating roundtrip latency and saving time for users. Format: scheme://domain.tld (one per line)', 'perfmatters')
        )
    );
...
function perfmatters_print_toggle($args) {
    if(!empty($args['section'])) {
        $section = $args['section'];
        $options = get_option($args['section']);
    }
    else {
        $section = 'perfmatters_options';
        $options = get_option('perfmatters_options');
    }
    if(!empty($args['section']) && $args['section'] == 'perfmatters_extras') {
        $extras = $options;
    }
    else {
        $extras = get_option('perfmatters_extras');
...
//print select option
function perfmatters_print_select($args) {
	$options = get_option('perfmatters_options');
	echo "<select id='" . $args['id'] . "' name='perfmatters_options[" . $args['id'] . "]'>";
		foreach($args['options'] as $value => $title) {
			echo "<option value='" . $value . "' "; 
...
function perfmatters_print_preconnect($args) {
    $extras = get_option('perfmatters_extras');
     echo "<div style='display: table; width: 100%;'>";
        echo "<div class='perfmatters-input-wrapper'>";
            echo "<textarea id='" . $args['id'] . "' name='perfmatters_extras[" . $args['id'] . "]' placeholder='https://example.com'>";
