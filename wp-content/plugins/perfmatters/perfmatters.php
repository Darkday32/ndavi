...
define('PERFMATTERS_VERSION', '1.3.5');
...
function perfmatters_menu() {
	if(perfmatters_network_access()) {
		$pages = add_options_page('perfmatters', 'Perfmatters', 'manage_options', 'perfmatters', 'perfmatters_admin');
	}
}
...
function perfmatters_meta_links($links, $file) {
	if(strpos($file, 'perfmatters.php' ) !== false) {
		...
		if(is_network_admin()) {
			...
			$license_url = network_admin_url('settings.php?page=perfmatters');
		}
		else {
			$license_info = perfmatters_edd_check_license();
			$license_url = admin_url('options-general.php?page=perfmatters');
		}

		$perfmatters_links = array();
		$perfmatters_links[] = '<a href="' . $license_url . '">' . esc_html__('Settings', 'perfmatters') . '</a>';
		...
		if(!is_plugin_active_for_network('perfmatters/perfmatters.php') || is_network_admin()) {
			...
			if(!empty($license_info->license) && $license_info->license == "valid") {
				$perfmatters_links[] = '<a href="' . $license_url . '&tab=license" style="color: green;">' . __('License is Activated', 'perfmatters') . '</a>';
			}
			elseif(!empty($license_info->license) && $license_info->license == "expired") {
				$perfmatters_links[] = '<a href="' . $license_url . '&tab=license" style="color: orange;">' . __('Renew License', 'perfmatters') . '</a>';
			}
